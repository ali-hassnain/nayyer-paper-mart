...
const MakePaymentForm = (props) => {
	...
	return (
		<div className='p-4 h-full overflow-y-scroll'>
			<div className='max-h-[80vh] mobile:max-h-[90vh] overflow-y-auto'>
				<div className='w-full max-w-[95vw] mobile:min-w-[300px] overflow-y-scroll h-[60vh]'>
					<Form
						...
						onSubmit={rhfHandleSubmit(async (formData) => {
							...
							try {
								...
								if (formData.payment_proof?.[0]) {
									proofUrl = await handleUploadImages(
										formData.payment_proof[0]
									);
								}
								...
								const { payment, error: paymentError } =
									await POST__createPayment(paymentPayload);
